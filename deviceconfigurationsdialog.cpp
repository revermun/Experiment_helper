...
/// OBSVM
/// BESTNAV
/// MODE ROVER/BASE/ROVER SURVEY/ROVER AUTOMOTIVE/UAV
/// UNIHEADING
///
...
/// Включена ли опция поиска решения RTK, DGNSS.
/// отправить/прочитать cfg-nav5 и cfg-dgnss, меняет dgnss timeout и diff mode соответственно +
/// Включено ли детектирование jamming’а?
/// отправить/прочитать cfg-itfm, он не реализован!!!!
/// Есть ли сообщения с эфемеридами, с какой частотой выдаются.
/// отправить/прочитать cfg-msg rxm-sfrbx +
/// Есть ли сообщения с наблюдениями, с какой частотой выдаются.
/// отправить/прочитать cfg-msg rxm-rawx  +
/// Есть ли сообщения со скоростью, позицией и т.д., с какой частотой выдаются, в каких единицах, в каких координатах.
/// отправить/прочитать cfg-msg  +
/// Режим работы: подвижный, стационарный и т.д.
/// отправить/прочитать cfg-nav5, меняет dynMod +
/// Относительное решение (относительно другой антенны)
/// отправить/прочитать cfg-msg для relposned +
    ...
    , ui(new Ui::deviceConfigurationsDialog)
{
    ...
    messagesIDMap["CLOCK"] = QPair<uint8_t,uint8_t>(NAV::CLOCK::classID, NAV::CLOCK::messageID);
    messagesIDMap["DGPS"] = QPair<uint8_t,uint8_t>(NAV::DGPS::classID, NAV::DGPS::messageID);
    messagesIDMap["DOP"] = QPair<uint8_t,uint8_t>(NAV::DOP::classID, NAV::DOP::messageID);
    messagesIDMap["ORB"] = QPair<uint8_t,uint8_t>(NAV::ORB::classID, NAV::ORB::messageID);
    messagesIDMap["POSECEF"] = QPair<uint8_t,uint8_t>(NAV::POSECEF::classID, NAV::POSECEF::messageID);
    messagesIDMap["POSLLH"] = QPair<uint8_t,uint8_t>(NAV::POSLLH::classID, NAV::POSLLH::messageID);
    messagesIDMap["RELPOSNED"] = QPair<uint8_t,uint8_t>(NAV::RELPOSNED::classID, NAV::RELPOSNED::messageID);
    messagesIDMap["SAT"] = QPair<uint8_t,uint8_t>(NAV::SAT::classID, NAV::SAT::messageID);
    messagesIDMap["SOL"] = QPair<uint8_t,uint8_t>(NAV::SOL::classID, NAV::SOL::messageID);
    messagesIDMap["STATUS"] = QPair<uint8_t,uint8_t>(NAV::STATUS::classID, NAV::STATUS::messageID);
    messagesIDMap["VELECEF"] = QPair<uint8_t,uint8_t>(NAV::VELECEF::classID, NAV::VELECEF::messageID);
    messagesIDMap["VELNED"] = QPair<uint8_t,uint8_t>(NAV::VELNED::classID, NAV::VELNED::messageID);

    messagesIDMap["DGNSS"] = QPair<uint8_t,uint8_t>(CFG::DGNSS::classID, CFG::DGNSS::messageID);
    messagesIDMap["CFG"] = QPair<uint8_t,uint8_t>(CFG::_CFG::classID, CFG::_CFG::messageID);
    messagesIDMap["GNSS"] = QPair<uint8_t,uint8_t>(CFG::GNSS::classID, CFG::GNSS::messageID);
    messagesIDMap["MSG"] = QPair<uint8_t,uint8_t>(CFG::MSG::classID, CFG::MSG::messageID);
    messagesIDMap["DAT"] = QPair<uint8_t,uint8_t>(CFG::DAT::classID, CFG::DAT::messageID);
    messagesIDMap["NAV5"] = QPair<uint8_t,uint8_t>(CFG::NAV5::classID, CFG::NAV5::messageID);
    messagesIDMap["PRT"] = QPair<uint8_t,uint8_t>(CFG::PRT::classID, CFG::PRT::messageID);
    messagesIDMap["RATE"] = QPair<uint8_t,uint8_t>(CFG::RATE::classID, CFG::RATE::messageID);
    messagesIDMap["RST"] = QPair<uint8_t,uint8_t>(CFG::RST::classID, CFG::RST::messageID);
    messagesIDMap["ITFM"] = QPair<uint8_t,uint8_t>(CFG::ITFM::classID, CFG::ITFM::messageID);

    messagesIDMap["RAWX"] = QPair<uint8_t,uint8_t>(RXM::RAWX::classID, RXM::RAWX::messageID);
    messagesIDMap["SFRBX"] = QPair<uint8_t,uint8_t>(RXM::SFRBX::classID, RXM::SFRBX::messageID);
...
//   0000010101
template<typename T>
T getBits(T num, int indexStart, int indexEnd){
    int numberOfBits = sizeof(num)*8 - 1;
...
void deviceConfigurationsDialog::parseMessage()
{
    ...
    if (dynamic_cast<CFG::MSG::MSGRATES*>(decodedMessage)){
        ...
        foreach (NAV::SAT::Repeated chData, info->repeatedList) {
            ...
            ui->tableDGPS->insertRow(i);
            QTableWidgetItem *item;
            item = new QTableWidgetItem(gnssId); ui->tableDGPS->setItem(i,0,item);
            item = new QTableWidgetItem(svId); ui->tableDGPS->setItem(i,1,item);
            item = new QTableWidgetItem(cno); ui->tableDGPS->setItem(i,2,item);
            item = new QTableWidgetItem(elevStr); ui->tableDGPS->setItem(i,3,item);
            item = new QTableWidgetItem(azimStr); ui->tableDGPS->setItem(i,4,item);
            item = new QTableWidgetItem(prRes); ui->tableDGPS->setItem(i,5,item);
            item = new QTableWidgetItem(qualityIndStr); ui->tableDGPS->setItem(i,6,item);
            item = new QTableWidgetItem(svUsed); ui->tableDGPS->setItem(i,7,item);
            item = new QTableWidgetItem(healthStr); ui->tableDGPS->setItem(i,8,item);
            item = new QTableWidgetItem(diffCor); ui->tableDGPS->setItem(i,9,item);
            item = new QTableWidgetItem(smothed); ui->tableDGPS->setItem(i,10,item);
            item = new QTableWidgetItem(orbitSourceStr); ui->tableDGPS->setItem(i,11,item);
            item = new QTableWidgetItem(eph); ui->tableDGPS->setItem(i,12,item);
            item = new QTableWidgetItem(alm); ui->tableDGPS->setItem(i,13,item);
            item = new QTableWidgetItem(ano); ui->tableDGPS->setItem(i,14,item);
            item = new QTableWidgetItem(aop); ui->tableDGPS->setItem(i,15,item);
            i++;
        }
        setupTableSize(ui->tableDGPS);
...
void deviceConfigurationsDialog::sendPoll()
{
    ...
    if (currentItemText == "CFG-MSG"){
        ...
        U1 classID = messagesIDMap[comboMSG->currentText()].first;
        U1 messageID = messagesIDMap[comboMSG->currentText()].second;
        ...
        qDebug() << messName;
        msg = UbloxParser::createPollMessage(messagesIDMap[messName].first, messagesIDMap[messName].second);
    }
...
void deviceConfigurationsDialog::sendSet()
{
    ...
    if (currentItemText == "CFG-MSG"){
        ...
        QByteArray payload;
        payload.resize(8);
        payload[0] = messagesIDMap[comboMSG->currentText()].first;
        payload[1] = messagesIDMap[comboMSG->currentText()].second;
...
void deviceConfigurationsDialog::sendSettings()
{
    ...
    if (protocol == "Ublox"){
        /// Что предполагается?
        /// Пользователю для начала нужно нажать кнопку обновить для того чтобы программа имела текущую информацию о приемнике
        /// затем при нажатии на кнопку отправить читаются поля в соответствующих фреймах и отправляются сообщения отличающиеся только заданными в первой вкладке значениями
        QByteArray msg;
        U1 hdrdata[] = {0xb5, 0x62};
        QByteArray hdr(reinterpret_cast<char*>(hdrdata), sizeof(hdrdata));
        U1 endata[4];
