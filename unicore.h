...
inline ULONG CalculateCRC32(const UCHAR *szBuf, INT iSize)
{
    int iIndex;
    ULONG ulCRC = 0;
...
inline ULONG CalculateCRC32(const std::string& str)
{
    return CalculateCRC32(reinterpret_cast<const UCHAR*>(str.c_str()),
                          static_cast<INT>(str.length()));
}

inline uint32_t rtk_crc32(const uint8_t *buff, int len)
{
    uint32_t crc=0;
    int i,j;
...
inline uint8_t calculateCommandChecksum(const uint8_t *buff, int len)
{
    uint32_t checksum=0;
    int i;
